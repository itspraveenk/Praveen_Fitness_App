...
function switchView(app, viewName) {
    ...
    if (viewName === 'newForm') {
        ...
        if (app === 'w') renderWorkoutsDashboard();
        if (app === 's') renderSpendingDashboard();
    }
}
...
function addExerciseBlock(exerciseData = null) {
    ...
    block.innerHTML = `
        ...
        <div class="sets-container">
            <div class="set-headers"><span>Set</span><span>lbs/kg</span><span>Reps</span><span></span></div>
...
function getDateFromISOWeek(weekStr) {
    ...
    return new Date(y, 0, 4 + (w - 1) * 7 - (new Date(y, 0, 4).getDay() || 7) + 1);
}
...
function initCustomWorkoutsAnalytics() {
    ...
    DOM.w.elements.customEndWeek.value = getISOWeekString(today);
    DOM.w.elements.customStartWeek.value = getISOWeekString(startObj);
...
function renderCustomWorkoutsChart() {
    ...
    const startDate = getDateFromISOWeek(startStr);
    const endDate = getDateFromISOWeek(endStr);
    ...
    while (curr <= endDate) {
        ...
        buckets[key] = { volume: 0, maxWeight: 0, found: false };
    ...
    if (filterEx === 'All') {
        ...
        chartOpts = {
            ...
            options: {
                ...
                maintainAspectRatio: false,
                scales: {
                    y: { beginAtZero: false, grid: { color: 'rgba(255, 255, 255, 0.05)' }, ticks: { callback: function (val) { return val + ' lbs'; } } },
                    x: { grid: { display: false } }
                },
                plugins: {
                    legend: { display: true, labels: { color: 'rgba(255, 255, 255, 0.7)' } },
                    tooltip: { callbacks: { label: function (ctx) { return ctx.raw ? ctx.raw + ' lbs' : 'No data'; } } }
                }
            }
        };
    }
...
function seedMockData() {
    if (workouts.length === 0) {
        const muscles = ['Biceps', 'Back', 'Chest', 'Legs', 'Shoulders', 'Triceps'];
        const exLibrary = {
            ...
            'Legs': ['Squat', 'Leg Press', 'Calf Raise', 'Romanian Deadlift'],
            'Shoulders': ['Overhead Press', 'Lateral Raise', 'Front Raise'],
            'Triceps': ['Tricep Pushdown', 'Skullcrusher', 'Close-Grip Bench']
        };

        for (let i = 0; i < 150; i++) {
            const d = new Date();
            d.setDate(d.getDate() - Math.floor(Math.random() * 180)); // last 6 months

            const m = muscles[Math.floor(Math.random() * muscles.length)];
            const chosenExercises = [];

            // Pick 2-4 random exercises for the muscle
            const numEx = Math.floor(Math.random() * 3) + 2;
            for (let j = 0; j < numEx; j++) {
                const exName = exLibrary[m][Math.floor(Math.random() * exLibrary[m].length)];
                if (!chosenExercises.find(e => e.title === exName)) {
                    // Generate 3-4 sets
                    const sets = [];
                    const numSets = Math.floor(Math.random() * 2) + 3;
                    // Base weight between 20-200 lbs depending on exercise
                    let baseWeight = Math.floor(Math.random() * 90) * 2 + 20;

                    for (let s = 0; s < numSets; s++) {
                        sets.push({
                            weight: baseWeight + (s * 5), // slight progression across sets
                            reps: Math.floor(Math.random() * 6) + 6 // 6-12 reps
                        });
                    }
                    chosenExercises.push({ title: exName, sets: sets });
                }
            }
    ...
    if (expenses.length === 0) {
        for (let i = 0; i < 40; i++) {
            const d = new Date();
            d.setDate(d.getDate() - Math.floor(Math.random() * 90)); // last 90 days
            const types = ['Groceries', 'Eating Out', 'Provisions', 'Travel'];
            expenses.push({
                id: Date.now().toString() + i,
                date: d.toISOString().split('T')[0],
...
// Seed the mock data on init (will only run if empty)
seedMockData();
...
function exportData() {
    ...
    a.href = url;
    a.download = `PraveenApp_Backup_${new Date().toISOString().split('T')[0]}.json`;
...
async function pushToGitHub() {
    ...
    try {
        const backupData = { workouts, expenses };
